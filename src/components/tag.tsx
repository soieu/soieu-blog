...
export default async function Tag({
  ...
  return params?.tag === tag.tag ? (
    ...
    <Link href={`/tags/${tag.tag}`}>
      <p className="border border-black rounded-full p-2 m-1 bg-white text-black font-bold hover:bg-black hover:text-white hover:cursor-pointer text-xs xl:text-base">
        # {tag.tag} {tag.count}
      </p>
    </Link>
  );
